...
function curlRequest($url, $curl_handle, $fail_on_error = false)
{
    ...
    curl_setopt($curl_handle, CURLOPT_USERAGENT, 'Hush Node Status Page');
...
function getData($from_cache = false)
{
    ...
    // Hush Daemon uptime
    if ($config['display_hushd_uptime'] === true) {
      if (strcmp(PHP_OS, "Linux") == 0) {
        $data['hushd_uptime'] = getLinuxProcessUptime($config['hushd_process_name']);
      }
      elseif (strcmp(PHP_OS,"Darwin") == 0) {
        $data['hushd_uptime'] = getDarwinProcessUptime($config['hushd_process_name']);
      }
